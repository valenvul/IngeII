...
public abstract class MutationAnalysisRunner {
     ...
     */
    protected Class<? extends Stack> classBeingTested;
    ...
    public boolean runTestsForCurrentClassBeingTested() {
        ...
        try {
            ...
            List<Pair<Class<?>, Method>> testMethods = getTestMethods();
            for (Pair<Class<?>, Method> testMethod : testMethods) {
                Method method = testMethod.getRight();

                // get instance as a class of MutationAnalysisRunner
                MutationAnalysisRunner instance = (MutationAnalysisRunner) testMethod.getLeft().newInstance();
                instance.classBeingTested = classBeingTested;
     ...
     */
    protected List<Pair<Class<?>, Method>> getTestMethods() {
        List<Pair<Class<?>, Method>> methods = new ArrayList<>();
        Method[] declaredMethods = this.getClass().getDeclaredMethods();
        for (Method method : declaredMethods) {
            methods.add(Pair.of(this.getClass(), method));
        }

        if (this.getClass().getSimpleName().equals("RegressionTest")) {
            Suite.SuiteClasses suiteClassesAnnotation = this.getClass().getAnnotation(Suite.SuiteClasses.class);
            if (suiteClassesAnnotation != null) {
                Class<?>[] suiteClasses = suiteClassesAnnotation.value();
                for (Class<?> suiteClass : suiteClasses) {
                    Method[] suiteClassMethods = suiteClass.getDeclaredMethods();
                    for (Method method : suiteClassMethods) {
                        methods.add(Pair.of(suiteClass, method));
                    }
                }
            }
        }
        ...
        return methods.stream()
                .filter(pair -> pair.getRight().getName().startsWith("test"))
     ...
     */
    String getMutantDescription(String mutantSimpleClassName) {
